...
def baseline_predictions(truth: np.ndarray, mask: np.ndarray, eligible: np.ndarray,
                         statistic: str) -> np.ndarray:
    observed = np.broadcast_to(eligible, mask.shape) & ~mask
    values = np.where(observed, np.asarray(truth), np.nan)
    # Ineligible native-vocabulary columns are deliberately all-NaN and are
    # never scored. Summarize only eligible columns to avoid misleading
    # all-NaN warnings while preserving NaN outside the evaluation universe.
    center = np.full(truth.shape[1], np.nan, dtype=np.float64)
    if statistic == "mean":
        center[eligible] = np.nanmean(values[:, eligible], axis=0)
    elif statistic == "median":
        center[eligible] = np.nanmedian(values[:, eligible], axis=0)
    else:
        raise ValueError(statistic)
    return np.broadcast_to(center, truth.shape)
...
def run_full() -> None:
    ...
    for mode in ("shared_vocab", "native_vocab"):
        for ratio in map(float, CONFIG["mask_ratios"]):
            for seed in map(int, CONFIG["mask_seeds"]):
                baseline_done = set()
                for model_name in MODELS:
                    ...
                    if space not in baseline_done:
                        for statistic in ("mean", "median"):
                            baseline_tag = (f"{mode}__gene_{statistic}__{space}__"
                            ...
                            if not baseline_output.is_file():
                                baseline = baseline_predictions(truth, mask, eligible, statistic)
                                score_prediction(
                                    sample_ids, truth, baseline, mask, benchmark=mode,
                                    method=f"gene_{statistic}", target_space=space,
    ...
    expected = 2 * len(CONFIG["mask_ratios"]) * len(CONFIG["mask_seeds"]) * 7
    if len(condition_files) != expected:
        raise RuntimeError(f"Expected {expected} condition tables, found {len(condition_files)}")
