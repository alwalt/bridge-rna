...
def main():
    ...
    elif is_main and HAS_WANDB and wandb.run is not None:
        run_id = wandb.run.id
    else:
    ...
    for epoch in range(start_epoch, CONFIG['epochs']):
        ...
        for batch_idx, (x_masked, x_true, mask_idx, species_id) in enumerate(train_loader):
            ...
            train_data_wait_s += max(0.0, step_start_t - prev_step_end_t)

            t0 = time.perf_counter()
            x_masked = x_masked.to(device)
            x_true = x_true.to(device)
            species_id = species_id.to(device)
            train_h2d_s += time.perf_counter() - t0

            t0 = time.perf_counter()
            pred = model(x_masked, species_id)  # [B, G]
            train_forward_s += time.perf_counter() - t0
            ...
            train_loss_s += time.perf_counter() - t0

            optimizer.zero_grad()
            t0 = time.perf_counter()
            loss.backward()
            train_backward_s += time.perf_counter() - t0

            t0 = time.perf_counter()
            optimizer.step()
            train_optim_s += time.perf_counter() - t0

            running_loss += loss.item()
            num_batches += 1
            step_end_t = time.perf_counter()
            train_step_s += step_end_t - step_start_t
            ...
            if is_main and (should_log_batch or should_log_time):
                avg = running_loss / num_batches
                avg_step = train_step_s / max(1, num_batches)
                elapsed = step_end_t - last_progress_log_t
                print(f"  Epoch {epoch+1}/{CONFIG['epochs']} | "
                      f"Batch {batch_idx+1}/{len(train_loader)} | "
                      f"Loss: {loss.item():.6f} | Avg: {avg:.6f} | "
                      f"Avg step: {avg_step:.3f}s | Window: {elapsed:.1f}s")
        ...
        if is_main:
            ...
            if timing_profile:
                ...
                print(
                    ...
                    f"bwd={train_backward_s/denom:.3f}, opt={train_optim_s/denom:.3f}"
                )
                print(f"  ║ Timing/validate total (s): {val_time_s:.1f}")
